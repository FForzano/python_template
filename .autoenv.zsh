...
# Environment variables for autoenv
if [[ -z "$AUTOENV_ENV_FILENAME" ]]; then
  autostash AUTOENV_ENV_FILENAME=".autoenv.zsh"
fi
if [[ -z "$AUTOENV_ENV_LEAVE_FILENAME" ]]; then
  autostash AUTOENV_ENV_LEAVE_FILENAME=".autoenv.zsh"
fi
if [[ -z "$AUTOENV_HANDLE_LEAVE" ]]; then
  autostash AUTOENV_HANDLE_LEAVE=1
fi
